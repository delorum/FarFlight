...
Голубые полосы спидометра и высотомера показывают диапазоны не хуже 95% от максимальной расчётной дальности для текущего направления и ветра. Для каждой высоты отдельно подбирается выгодная скорость. На земле рекомендации не показываются; расчёт обновляется не чаще раза в реальную секунду.
...
Под основным высотомером `РВ … м` показывает высоту над землёй непосредственно под самолётом. Ниже 100 м показание становится красным. Радиовысотомер работает при включённом питании до 750 м включительно; выше выводится `РВ —`. Он не показывает рельеф впереди.
