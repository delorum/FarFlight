...
func update(delta: float) -> void:
	...
	if stalled:
		...
		# During rotation and the first seconds after liftoff, limit nose-up
		# attitude. This prevents a fully held yoke from commanding an immediate
		# 18-degree pitch before the flight path and airspeed can catch up.
		var pitch_command_limit := 10.0 if state == State.PARKED or takeoff_grace_remaining > 0.0 else 18.0
		pitch_deg = move_toward(pitch_deg, yoke.y * pitch_command_limit, delta * 30.0)
	...
	# Losing altitude converts potential energy into airspeed. Four km/h of
	# target-speed gain per m/s of descent keeps a normal glide controllable,
	# while a steep dive produces an unmistakable acceleration.
	var gravity_dive_bonus: float = maxf(0.0, -vertical_speed_mps) * 4.0
	var target_speed: float = glide_speed + throttle * 190.0 * altitude_power_factor() - climb_drag - dive_drag - unsupported_climb * 8.0 + gravity_dive_bonus
...
func _try_land() -> bool:
	for i in world.airports.size():
		...
		if abs(coords.x) <= FlightWorldScript.RUNWAY_LENGTH_KM * 0.5 and abs(coords.y) <= FlightWorldScript.RUNWAY_WIDTH_KM * 0.5 and aligned and speed_kmh < 100.0 and vertical_speed_mps <= 0.0 and vertical_speed_mps > -5.5:
			state = State.ROLLING
			airport_index = i
			altitude_m = 0.0
			vertical_speed_mps = 0.0
			...
			stalled = false
			wheel_brakes_applied = false
			_show_message("Касание ВПП «%s» — газ 0%%, удерживайте S для торможения" % airport.name, -1.0)
			return true
	return false
...
func _update_ground_roll(delta: float) -> void:
	...
	speed_kmh = clampf(speed_kmh + acceleration_kmh_s * delta, 0.0, 200.0)
...
func _landing_failure_reason() -> String:
	...
	if speed_kmh >= 100.0:
		failures.append("скорость %.1f км/ч" % speed_kmh)
