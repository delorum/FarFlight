...
func _run() -> void:
	...
	assert(is_equal_approx(calc.values.time, 20.0) and is_equal_approx(calc.values.distance, 50.0))
	...
	assert(is_equal_approx(calc.values.altitude, 2200.0) and is_equal_approx(calc.values.time, 5.0))
	calc._set_value("time", 10.0)
	...
	assert(calc.valid and is_equal_approx(calc.values.time, 0.0))
	assert(is_equal_approx(calc.values.distance, 0.0) and is_equal_approx(calc.values.altitude, calc.initial_altitude))
