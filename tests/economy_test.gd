...
func _init() -> void:
	...
	assert(is_equal_approx(economy.fill_carried_canister(19.7), 19.7))
	...
	assert(needs.buy_hotel_rest())
	assert(needs.fatigue == 3 and needs.money == money_before_hotel - Economy.HOTEL_REST_PRICE)
