...
func _run() -> void:
	...
	assert(Calculator.calculate(parameters, "time", 1500).values.distance == 0.0)
	...
	assert(widget.valid and widget.last_changed == "time", "Entering zero vertical speed must release the target altitude")
	assert(is_equal_approx(widget.values.time, 10.0) and is_equal_approx(widget.values.altitude, 3000.0))
	assert(is_equal_approx(widget.values.distance, 30.0), "Levelling off must preserve time and distance")
	widget._set_value("vertical", -2.0)
	widget._set_value("altitude", 1800.0)
	scene.flight.vertical_speed_mps = 0.0
	widget.current_buttons.vertical.pressed.emit()
	assert(widget.valid and is_equal_approx(widget.values.altitude, widget.initial_altitude), "Current vertical speed button must also support level flight")
	widget._set_value("vertical", -0.1)
	widget._set_value("altitude", 1800.0)
	widget.fields.vertical.gui_input.emit(wheel)
	assert(widget.valid and is_equal_approx(widget.values.altitude, widget.initial_altitude), "Wheel to zero must also recalculate final altitude")
	widget._set_value("vertical", -2.0)
	widget._set_value("altitude", 1800.0)
	widget._set_value("vertical", -1.0)
	assert(is_equal_approx(widget.values.altitude, 1800.0) and is_equal_approx(widget.values.time, 20.0), "Changing descent rate must preserve the selected target altitude")
	widget.fields.vertical.text_changed.emit("2")
	assert(widget.valid and widget.last_changed == "time")
	assert(is_equal_approx(widget.values.time, 20.0) and is_equal_approx(widget.values.altitude, 5400.0), "Switching from descent to climb must preserve time and raise the destination altitude")
	widget._set_value("altitude", 5400.0)
	scene.flight.vertical_speed_mps = -1.0
	widget.current_buttons.vertical.pressed.emit()
	assert(widget.valid and is_equal_approx(widget.values.time, 20.0) and is_equal_approx(widget.values.altitude, 1800.0), "Switching from climb to descent via current speed must lower the destination altitude")
	widget._set_value("time", 5.0)
	assert(is_equal_approx(widget.values.distance, 15.0) and is_equal_approx(widget.values.altitude, 2700.0))
	widget._set_value("speed", 120.0)
	assert(is_equal_approx(widget.values.time, 5.0) and is_equal_approx(widget.values.distance, 10.0), "Changing speed must preserve time when time was the last selected constraint")
	...
	widget._set_value("vertical", 2.0)
	assert(widget.valid and is_equal_approx(widget.values.altitude, 4000.0), "A corrected climb rate must resolve the pending target altitude")
	...
	assert(is_equal_approx(widget.values.altitude, 4000.0) and is_equal_approx(widget.values.time, 5.0), "Explicit altitude sampling must preserve the target and recalculate time")
	widget._set_value("time", 10.0)
	scene.flight.altitude_m = 3200.0
	await process_frame
	assert(is_equal_approx(widget.values.altitude, 4000.0), "Time-based plans must also remain fixed while flying")
	widget.current_buttons.initial_altitude.pressed.emit()
	assert(is_equal_approx(widget.values.time, 10.0) and is_equal_approx(widget.values.altitude, 4000.0) and is_equal_approx(widget.values.vertical, 800.0 / 600.0), "Explicit sampling must preserve recently entered time and derive vertical speed")
	...
	assert(is_equal_approx(widget.initial_altitude, 2100.5) and is_equal_approx(widget.values.altitude, 4000.0), "Initial altitude must apply typed decimal values without Enter")
	...
	widget._set_value("wind_speed", 0.0)
	# Pick a finished segment without starting a new annotation.
	...
	selection.position = scene.world_to_screen((a + b) / 2)
	selection.pressed = true
	scene._handle_mouse_button(selection)
	selection.pressed = false
	scene._handle_mouse_button(selection)
	assert(scene.pending_measure == null and scene.measurement_lines.size() == 1)
	...
	widget.use_line(a, b)
	assert(not widget.heading_based and is_equal_approx(widget.values.track, 90.0), "Picking a line must use its direction, not the previous manual heading")
