...
func _initialize() -> void:
	...
	var repeated_weather_world = World.new(424242)
	repeated_weather_world.refresh_weather()
	check(repeated_weather_world.weather_generation == 1 and repeated_weather_world.wind_layers == shape_world.wind_layers and repeated_weather_world.storms == shape_world.storms, "The same seed and weather cycle must reproduce wind and storms")
	var weather_snapshot := shape_world.snapshot()
	var restored_weather_world = World.new(424242)
	restored_weather_world.restore_snapshot(weather_snapshot)
	shape_world.refresh_weather()
	restored_weather_world.refresh_weather()
	check(restored_weather_world.weather_generation == 2 and restored_weather_world.wind_layers == shape_world.wind_layers and restored_weather_world.storms == shape_world.storms, "A saved weather generation must continue the deterministic sequence")
